...
`)}\n`,t.scrollTop=t.scrollHeight)},echo(e){this.print([`> ${e}`])},clear(){t&&(t.textContent=``,t.scrollTop=0)},focus(){n&&n.focus()},update({room:e,state:t,world:n}){l(e,t),u(e,t,n)}}}async function Xt(){let e=Ye(await n(),{randomEvents:!0,debugInventory:q()}),t=e=>{},r=qt(document.getElementById(`scene`),document.getElementById(`sceneLabel`),document.getElementById(`sceneOverlay`),e=>t(e)),i=ot(document.getElementById(`scene`),document.getElementById(`log`)),a=document.getElementById(`mapToggle`),o=document.getElementById(`artToggle`),s=document.getElementById(`artToggleLabel`);function c(){if(!o)return;let e=r.getArtMode()===`classic`;o.classList.toggle(`active`,e),s&&(s.textContent=e?`1982`:`CLASSIC`),o.title=e?`Switch to illustrated graphics`:`Switch to 1982 Apple II graphics`}o&&(o.addEventListener(`click`,()=>{r.toggleArtMode(),c()}),c());let l=document.getElementById(`debugToggle`);l&&(l.classList.toggle(`active`,q()),l.addEventListener(`click`,()=>{et(),location.reload()}));let u=_t({miniMapElement:document.getElementById(`miniMap`),miniMapViewport:document.getElementById(`miniMapViewport`),miniMapBadge:document.getElementById(`miniMapBadge`),drawerElement:document.getElementById(`mapDrawer`),drawerBody:document.getElementById(`mapDrawerBody`),drawerStats:document.getElementById(`mapDrawerStats`),drawerClose:document.getElementById(`mapDrawerClose`),drawerBackdrop:document.getElementById(`mapDrawerBackdrop`)});a&&a.addEventListener(`click`,()=>{u.toggle()});function d(){u.update({currentRoom:e.state.room,visitedRooms:e.getVisitedRooms(),world:e.world,state:e.state})}let f=ut({overlayElement:document.getElementById(`gameOverOverlay`),onRestart:()=>t(`restart`)}),p,m=()=>{let t=e.world.room(e.state.room);if(r.show(t,e.state,e.world),d(),p&&p.update({room:t,state:e.state,world:e.world}),e.isGameOver()){let t=Xe(e.state);f.show(t)}else f.hide()},h=Qe({engine:e,onJump:m}),g=e.state.room;function _(){e.state.room!==g&&(g=e.state.room,p.clear())}let v=document.getElementById(`turnBadge`);function y(){v&&(v.textContent=`TURN ${e.state.turns}`)}t=function(t){e.isGameOver()&&(t=`restart`);let n=t.trim().toLowerCase();n===`map`||n===`m`?u.open():(n===`scene`||n===`close`&&u.isOpen())&&u.close();let r=h.handle(t);if(r!==null){Promise.resolve(r).then(e=>{_(),p.echo(t),p.print(e),y(),d(),m()});return}i.prime();let{echo:a,messages:o,events:s}=e.execute(t);a!==null&&(_(),p.echo(a),p.print(o),m(),y(),s?.length&&i.play(s))},window.addEventListener(`keydown`,n=>{e.isGameOver()&&([`Shift`,`Control`,`Alt`,`Meta`,`CapsLock`,`Tab`].includes(n.key)||((n.key===` `||n.key===`Enter`)&&n.preventDefault(),t(`restart`)))}),p=Yt({onCommand:t}),p.print(e.start()),y(),d();let b=h.initialRoom();b!==null&&(e.state.room=b,_(),p.print([`DEBUG: resumed at room ${b}`,e.world.room(b).desc])),m(),p.focus()}Xt().catch(e=>{console.error(e);let t=document.getElementById(`log`);t&&(t.textContent+=`\nFAILED TO START: ${e.message}\n`)});